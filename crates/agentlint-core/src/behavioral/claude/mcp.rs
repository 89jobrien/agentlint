...
pub fn looks_like_plaintext_secret(s: &str) -> bool {
...
pub fn find_duplicate_server_names(src: &str) -> Vec<String> {
...
mod tests {
    ...
    fn op_uri_in_env_is_warning() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.message.contains("op://") && d.severity == Severity::Warning),
    ...
    fn fetch_server_name_is_warning() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.rule == "claude/mcp/fetch-server" && d.severity == Severity::Warning),
