use super::mcp::{find_duplicate_server_names, validate_server_entry};
...
mod tests {
    use super::*;
    use crate::Severity;
    use crate::testing::{assert_clean, assert_error_contains};
    use std::path::Path;
    ...
    fn hook_with_python3_is_warning() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.message.contains("python3") && d.severity == Severity::Warning),
    ...
    fn hook_with_sleep_is_error() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.message.contains("sleep") && d.severity == Severity::Error),
    ...
    fn hook_with_cargo_is_warning() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.message.contains("cargo") && d.severity == Severity::Warning),
    ...
    fn allow_sshpass_credential_is_error() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.message.contains("sshpass -p") && d.severity == Severity::Error),
