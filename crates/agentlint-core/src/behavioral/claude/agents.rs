...
impl AgentsValidator {
    pub fn validate(path: &Path, src: &str) -> Vec<Diagnostic> {
        ...
        for &req in REQUIRED_FIELDS {
            match fields.iter().find(|f| f.key == req) {
                None => {
                ...
                Some(f) if f.value.is_empty() => {
                    diags.push(Diagnostic::error(
                        ...
                        f.line,
                        1,
                        format!("required field '{req}' must not be empty"),
                    ));
                }
                Some(_) => {}
            }
        ...
        for field in &fields {
            match field.key.as_str() {
                ...
                "description"
                    if !field.value.is_empty() && field.value.len() < MIN_DESCRIPTION_LEN =>
                {
                    diags.push(
                        Diagnostic::warning(
                            path,
...
pub fn check_duplicate_names<'a>(files: &[(&'a Path, &'a str)]) -> Vec<Diagnostic> {
    ...
    for (path, src) in files {
        if let Ok(fields) = frontmatter::parse(src) {
            for field in &fields {
...
mod tests {
    use super::*;
    use crate::testing::{assert_clean, assert_error_at, assert_error_contains};
